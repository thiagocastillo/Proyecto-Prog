...
public class Mapa
{
    ...
    // Muestra el mapa en la consola con colores para cada jugador
public void MostrarMapaConColores(List<Jugador> jugadores)
{
    char[,] grid = new char[Alto, Ancho];
    int?[,] propietario = new int?[Alto, Ancho]; // Guarda el índice del jugador dueño
    ...
    // Edificios y unidades de jugadores
    ...
    // Imprime el mapa en consola con colores
    for (int y = 0; y < Alto; y++)
    {
        for (int x = 0; x < Ancho; x++)
        {
            if (propietario[y, x] == 0)
                Console.ForegroundColor = ConsoleColor.Blue;
            else if (propietario[y, x] == 1)
